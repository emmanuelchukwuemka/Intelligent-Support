...
-- PostgreSQL schema (Chapter 4.4.2 of the report, adapted to Postgres)

DROP TABLE IF EXISTS feedback CASCADE;
DROP TABLE IF EXISTS user_interventions CASCADE;
DROP TABLE IF EXISTS progress_tracking CASCADE;
DROP TABLE IF EXISTS stress_assessments CASCADE;
...
CREATE INDEX idx_assessments_user ON stress_assessments(user_id);
CREATE INDEX idx_progress_user ON progress_tracking(user_id);
CREATE INDEX idx_user_interventions_user ON user_interventions(user_id);
CREATE INDEX idx_user_interventions_intervention ON user_interventions(intervention_id);

-- ---------------------------------------------------------------------
-- Seed data: interventions, mapped to the stress "category" themes the
-- ML classifier derives from the DASS-21-style questionnaire, and to
-- severity_level so the recommendation engine can content-filter on
-- both axes before/while collaborative filtering has enough data.
-- ---------------------------------------------------------------------
INSERT INTO interventions (intervention_name, intervention_type, description, target_severity, effectiveness_rating) VALUES
('5-Minute Box Breathing', 'Anxiety', 'A short paced-breathing exercise (4s in, 4s hold, 4s out, 4s hold) to calm the nervous system.', 'Low', 4.1),
...
('Academic/Work Load Re-negotiation Plan', 'Workload', 'A structured template for discussing deadline extensions or workload reduction with supervisors/tutors.', 'Very High', 4.1),
...
('Structured Exercise Program (3x/week)', 'Physical', 'A beginner strength + cardio program to build long-term resilience to stress.', 'High', 4.3),
...
('Problem-Solving Coping Worksheet', 'Coping', 'A structured worksheet (Lazarus & Folkman problem-focused coping) to break down a stressor into actionable steps.', 'Moderate', 4.0),
('Emotion-Focused Coping Toolkit', 'Coping', 'A toolkit of emotion-focused coping techniques (relaxation, reframing, social support seeking).', 'High', 4.1),
...
('Understanding Stress: The Transactional Theory', 'Education', 'Stress occurs when you appraise a situation as exceeding your coping resources (Lazarus & Folkman, 1984). Primary appraisal asks "is this a threat?"; secondary appraisal asks "can I handle it?" Understanding this can help you separate the stressor from your reaction to it.'),
('Problem-Focused vs Emotion-Focused Coping', 'Education', 'Problem-focused coping targets the source of stress directly (planning, problem-solving). Emotion-focused coping manages your reaction (relaxation, reframing, social support). Most people need both, depending on whether the stressor is within their control.'),
('Why Sleep and Stress Feed Each Other', 'Sleep', 'Chronic stress raises cortisol, which disrupts sleep; poor sleep then reduces your capacity to cope with stress the next day. Breaking this cycle usually starts with consistent sleep/wake times, not just "more sleep".'),
('The Yerkes-Dodson Law: Stress Isn''t Always Bad', 'Education', 'A moderate amount of stress (arousal) improves performance up to a point, after which performance declines (Yerkes & Dodson, 1908). The goal of stress management isn''t zero stress -- it''s staying in your productive zone.'),
('Recognizing Burnout Early', 'Emotional Exhaustion', 'Burnout shows up as emotional exhaustion, cynicism/detachment, and reduced sense of accomplishment. Catching it early, before it becomes chronic, makes recovery significantly faster.'),
('Building a Sustainable Study/Work Schedule', 'Time Management', 'Workload stress often comes from underestimating task duration. A simple fix: estimate how long a task will take, then add 25-50% buffer, and block it on a calendar rather than keeping it as an open-ended to-do item.');
